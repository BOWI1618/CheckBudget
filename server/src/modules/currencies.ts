...
export const currencyRoutes = async (app: FastifyInstance): Promise<void> => {
  app.get('/currencies', async () => ({
    items: (await db.all<{ code: string; name_ru: string; symbol: string; exponent: number }>(
      'SELECT code, name_ru, symbol, exponent FROM currencies WHERE is_active = TRUE ORDER BY code',
  ...
  app.get<{ Querystring: { base?: string; on?: string } }>('/rates', async (req) => {
    ...
    const items = await db.all<{ quote_code: string; rate_num: number; rate_den: number; valid_on: string; source: string }>(
