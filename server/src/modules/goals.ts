...
export const listGoals = async (budgetId: string): Promise<GoalRow[]> =>
  await db.all<GoalRow>(
    'SELECT * FROM goals WHERE budget_id = ? AND deleted_at IS NULL ORDER BY due_on IS NULL, due_on, name',
    budgetId,
  );

async function findGoal(budgetId: string, id: string): Promise<GoalRow> {
...
export const goalRoutes = async (app: FastifyInstance): Promise<void> => {
  ...
  app.get<{ Params: { budgetId: string } }>('/budgets/:budgetId/goals', async (req) => {
    ...
    return { items: (await listGoals(member.budgetId)).map(toGoal) };
  });
  ...
  app.post<{ Params: { budgetId: string } }>('/budgets/:budgetId/goals', async (req, reply) => {
    ...
    return idempotent(
      ...
      () => mutate(member.userId, async (emit) => {
        ...
        const row = toGoal(await findGoal(member.budgetId, id));
        await emit({ budgetId: member.budgetId, entity: 'goal', entityId: id, op: 'insert', actorId: member.userId, payload: row });
        return row;
      }),
      201,
    );
  ...
  app.patch<{ Params: { budgetId: string; id: string } }>(
    ...
    async (req, reply) => {
      ...
      return idempotent(req, reply, () =>
        mutate(member.userId, async (emit) => {
          const current = await findGoal(member.budgetId, id);
          ...
          if (changes === 0) throw new VersionConflictError(toGoal(await findGoal(member.budgetId, id)));
          const row = toGoal(await findGoal(member.budgetId, id));
          await emit({ budgetId: member.budgetId, entity: 'goal', entityId: id, op: 'update', actorId: member.userId, payload: row });
          return row;
        }),
      );
    },
    ...
    async (req, reply) => {
      ...
      return idempotent(req, reply, () =>
        mutate(member.userId, async (emit) => {
          ...
          await emit({ budgetId: member.budgetId, entity: 'goal', entityId: id, op: 'delete', actorId: member.userId, payload: { id } });
          return { ok: true };
        }),
      );
    },
