...
export const config = {
  ...
  /**
   * Сколько месяцев операций попадает в снимок.
   *
   * Не «сколько влезет»: за пределами окна клиент дозагружает данные сам,
   * а графики за полгода строятся из месячных итогов, которые снимок отдаёт
   * отдельно. Три месяца покрывают всё, что интерфейс показывает сразу.
   */
  snapshotMonths: Number(env.SNAPSHOT_MONTHS ?? 3),
