...
async function main(): Promise<void> {
  ...
  db.tx(() => {
    for (const [id, email, name] of [
      ...
      db.run(
        `INSERT OR IGNORE INTO users (id, email, email_lower, password_hash, display_name, created_at, updated_at)
         VALUES (?,?,?,?,?,?,?)`,
        id, email, email, password, name, ts, ts,
      );
      db.run(
        `INSERT OR IGNORE INTO user_settings (user_id, base_currency, locale, theme, updated_at)
    ...
    db.run('INSERT INTO budget_members (budget_id, user_id, role, joined_at) VALUES (?,?,?,?)',
      budgetId, owner, 'owner', ts);
    db.run('INSERT INTO budget_members (budget_id, user_id, role, joined_at) VALUES (?,?,?,?)',
      budgetId, partner, 'editor', ts);

    seedBudgetDefaults(budgetId, 'RUB');
    ...
    const accounts = db.all<{ id: string; currency: string; name: string }>(
    ...
    db.run('UPDATE accounts SET initial_balance_minor = ? WHERE id = ?', parseAmount('310000', 'RUB'), card.id);
    db.run('UPDATE accounts SET initial_balance_minor = ? WHERE id = ?', parseAmount('12000', 'RUB'), cash.id);

    const categories = db.all<{ id: string; name: string; kind: string }>(
    ...
    const insertTx = (
      ...
      const conv = convertToBase(amountMinor, currency, 'RUB', occurredOn);
      db.run(
    ...
    for (let monthsAgo = 5; monthsAgo >= 0; monthsAgo--) {
      ...
      insertTx('income', card.id, byName.get('Зарплата')!.id,
      ...
      if (monthsAgo % 2 === 0) {
        insertTx('income', card.id, byName.get('Подработка')!.id,
      ...
      for (const [categoryName, note, min, max, count] of PATTERNS) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const account = Math.random() < 0.12 ? cash : card;
          insertTx('expense', account.id, category.id,
      ...
      insertTx('expense', usdAccount, byName.get('Техника')!.id,
    ...
    db.run('UPDATE user_settings SET default_budget_id = ? WHERE user_id IN (?, ?)', budgetId, owner, partner);
  });

  const count = db.get<{ n: number }>('SELECT COUNT(*) AS n FROM transactions')!.n;
